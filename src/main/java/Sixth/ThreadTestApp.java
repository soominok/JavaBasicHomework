...
class Car implements Runnable {
    ...
    public void run() {
        try {
            // 현재 스레드로 구동되는 객체를 강제로 sleepTime만큼 sleep (잠시 휴업)시킨다.
...
public class ThreadTestApp {
    public static void main(String[] args) {
        // Thread를 사용하려면 아래와 같이 스레드 객체가 될 대상을 지정해야 한다.
        // 추가로 스레드가 도리 대상 객체는 반드시 Runnable을 implemens 해야 한다.
        // 그래야 Thread를 구동시킬 수 있기 때문이다.

        // new Car가 Runnable을 implements해야 함.
        Thread t1 = new Thread(new Car("Porshe 919"));
...
// 스핀락은 컨텍 스위치을 하지 않고 폴링(계속 확인)
// 세마포어는 다른 일을 하러 감
// 이 때 이동을 해야하는데 이 경우는 컨텍스 스위칭이 일어난 것.
